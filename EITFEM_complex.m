...
%own cells.
    ...
    methods
        ...
        function SetInvGamma(self, meas_noise_coef_e, meas_noise_coef2)
            ...
            var_meas(select) = var_meas(select) + (meas_noise_coef2(1)*(abs(meas(select)) - min(abs(meas(select))))).^2;
            %imag:
            select = nmeas/2+1:nmeas;
            var_meas(select) = (meas_noise_coef_e(2)*(max(abs(meas(select)))-min(abs(meas(select)))))^2;
            var_meas(select) = var_meas(select) + (meas_noise_coef2(2)*(abs(meas(select)) - min(abs(meas(select))))).^2; %Assume no cross-correlations, hence a diagonal covariance mat:
