...
%Author: Petri Kuusela, 31.5.2024
    ...
    properties
        ...
        convergencePlotInterval %Convergence plots iteration-window size (negative value means show all values except -convergencePlotInterval from beginning. positive values mean show convergencePlotInterval last values. 0 means show all)
        ...
        useLastDist     %A flag, start the linesearch at relative distance of last minimum. Sometimes speeds up computations, sometimes causes issues.
    end
    ...
    methods
        ...
        function reco = Solve(self, sigEst)
            ...
            lastd = 1; %lastd contains the relative distance of minimum in last linesearch, if self.useLastDist == 1. Start with lastd = 1
            ...
            while (iter <= self.maxIter)
                ...
                while cont == 1%This is the loop for the linesearch
                    ...
                    if (ii > self.maxIterInLine)
                        ...
                        %converges, you can uncomment the next line to get a
                ...
                if self.useLastDist%we want to start the next linesearch at the relative distance of the minimum of this linesearch
                    lastd = d(minii);
                else
                    lastd = 1;
                end
                ...
                if self.plotConvergence%Plot the convergence plots
                    self.ConvergencePlots(self.convergencePlotInterval);
                end
                drawnow;
        ...
        function DataPlots(self, sigEst, iter)
            %This function calls the .Plot function with argument sigEst of each oFun whose
            %index is listed in self.plotDataInd. The main aim of this is
            %to plot the data fitting during the GN.
            for iplot = 1:length(self.plotDataInd)
                if isempty(self.hData)%This has to be done separately, because otherwise Matlab converts the figure handle into a double for some reason...
                    self.hData = figure();
                elseif length(self.hData) < iplot || isempty(self.hData(iplot))
                ...
                self.hData(iplot).Name = ['Iteration ', num2str(iter)];%mark down the iteration too on this window
